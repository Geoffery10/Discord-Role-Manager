...
async def update_database(members, guild):
    ...
    for member in members:
        ...
        if user is None:
            ...
            add_user(member.id, member.name, "00-00", member.discriminator)
            print("Added user: " + member.name)
        else:
            # Check if the user's tag is different
            if user.tag != member.discriminator:
                # Replace the tag in the database with the new tag
                update_user_tag(member.id, member.discriminator)
            ...
            if user.username != member.name:
                ...
                update_user_username(member.id, member.name)
                print("Updated username for user: " + member.name)

        # Check if user_id is in the user_guilds table
        if not is_user_in_guild(member.id, guild.id):
            # If user_id is not in the database, then add it
            add_user_to_guild(member.id, guild.id)
